...
import { UserParameter } from '../ui/UserParameter/UserParameter'
import { ButtonsWrapper } from '../ui/ButtonsWrapper/ButtonsWrapper'

export default function ProileUserBlock() {
    const [newArray, setNewArray] = useState<string | null>('https://4kwallpapers.com/images/walls/thumbs_3t/8351.jpg')
    const [newBack, setNewBack] = useState<string | null>('https://4kwallpapers.com/images/walls/thumbs_3t/12181.jpg')
    ...
    const handleAvatarUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event?.target?.files && event?.target?.files[0]){
            ...
            reader.onload = function () {
                ...
                setNewArray(reader.result as string)
            }
    ...
    const handleBackgroundUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
      if (event?.target?.files && event?.target?.files[0]){
          ...
          reader.onload = function () {
              console.log(reader.result);
              setNewBack(reader.result as string)
          }
          reader.onerror = function (error){
              console.log("Error", error);
  ...
  return (
    ...
    <div className="user__block">
      <UserAvatar userAvatarUrl={newArray} isProfile={true} onAvatarChange={handleAvatarUpload}/>
      <UserParameter/>
      {/*  */}
      <ButtonsWrapper/>

    </div>
    <UserBackground userBackgroundUrl={newBack} isBackground={true} onBackgroundChange={handleBackgroundUpload}/>
  </div>
  )
}
